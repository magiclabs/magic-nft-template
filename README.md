...
[Magic](https://magic.link) makes it super simple to spin up secure, non-custodial wallets for users. This NFT Starter Pack illustrates how to integrate [Magic Connect](https://magic.link/connect) with a basic NFT site that allows users to mint and view NFTs from a particular NFT collection. The guide below walks through the Magic Connect integration as well as the remaining items that may be new for developers just getting into Web3 (e.g. connecting to the Ethereum network, reading data from the network, submitting transactions, etc.)

## Getting Started

Before everything, let's make sure you can run the project locally. This is a simple [Next.js](https://nextjs.org/) app with a few basic dependencies. If you need to get familiar with Next.js, check out the [Next.js documentation](https://nextjs.org/docs/).
...
You'll then need to copy rename `.env.example` to `.env` and fill in the values it mentions. You'll need to get a `MAGIC_SECRET_KEY` and `NEXT_PUBLIC_MAGIC_PUBLISHABLE_KEY` from your [Magic Dashboard](https://dashboard.magic.link). You can sign up for a free account if you haven't already. The final value is the NFT contract address that the site uses for minting NFTs. You can use ours (`0x56A4664940E54920fF12C24030aCe0d6a333187d`) for testing on Goerli testnet if you'd like, but eventually you should replace it with your own contract address.
...
The remainder of the app uses the Wallet module `magic.wallet`. Specifically, the following four methods:

1. [`connectWithUI()`](https://magic.link/docs/connect/wallet-api-reference/javascript-client-sdk#connectwithui()) - displays the Magic Connect UI for sign up and authentication. Gets called when the user logs in or signs up. You can see example usage in `src/components/AppHeader.jsx` and `src/components/LoginWithMagic.jsx`.
...
3. [`showUI()`](https://magic.link/docs/connect/wallet-api-reference/javascript-client-sdk#showui()) - once you're connected, you can use this method to show the wallet UI. User interaction is then handled by the SDK.

    ```jsx
    magic.wallet.showUI().catch((err) => console.error(err));
    ```

4. [`disconnect()`](https://magic.link/docs/connect/wallet-api-reference/javascript-client-sdk#disconnect()) - disconnects from the user's wallet
...
Once you've used the Magic SDK to connect, you can interact with the network using popular web3 provider libraries like `web3.js` or `ethers.js`. You can see all of the EVM RPC methods that Magic supports [here](https://magic.link/docs/connect/wallet-api-reference/javascript-client-sdk#evm-rpc-methods). In this project, we only use the following methods:
...
Then you issue the actual transaction by calling the same method name and adding `.send`. `send` lets you add configuration options. We add the user's address and the estimated gas.
